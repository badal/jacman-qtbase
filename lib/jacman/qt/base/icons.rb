...
module JacintheManagement
  module GuiQt
    # Icons resources for Jacinthe
    module Icons
      ICON_FILES = File.join(File.dirname(__FILE__), 'icons')

      # @param [String] name name of icon
      # @return [Qt::Icon] trolltech icon
      def self.icon(name)
        filename = ":/trolltech/styles/commonstyle/images/#{name}-32.png"
        Qt::Icon.new(filename)
      end

      # @param [String] filename file in ICON_FILES directory
      # @return [Qt::Icon] trolltech icon
      def self.from_file(filename)
        path = File.join(ICON_FILES, filename)
        fail "Icon file #{path} does not exist" unless File.exist?(path)
        Qt::Icon.new(path)
      end
    end
  end
